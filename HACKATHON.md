...
| Requirement | Status | Implementation |
|-------------|--------|----------------|
| Claim sandbox capital from vault | ✅ Done | Called `claimAllocation(19)` on startup |
...
| Stop-out grace period | ✅ Done | 5-min immunity to stop-loss checks on new positions |
...
| Live dashboard | ✅ Done | PRISM React dashboard on port 3000 |
...
| Governance pipeline visualization | ✅ Done | 6-stage event-driven status from checkpoint types |
...
- Signals fire every 60s across **8 symbols** (BTCUSD, ETHUSD, SOLUSD, XMRUSD, ATOMUSD, LINKUSD, DOGEUSD, PEPEUSD)
- Every signal submits a signed TradeIntent on-chain regardless of execution mode
- Paper mode allows trading without Kraken API credentials
- 3 strategies independently evaluate every symbol → more signal coverage
- Adaptive learning improves win rate over time → better P&L
- SAGE reflection engine tunes ensemble weights → higher quality signals
...
| `On-chain attest failed` for Validation Registry | Non-blocking — checkpoints still saved locally and to IPFS |
| `TradeIntent failed: No wallet or router address` | Expected in paper mode without `PRIVATE_KEY` set |

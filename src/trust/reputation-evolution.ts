/**
 * Reputation Evolution — Capital Trust Tier Ladder
 *
 * The agent's capital rights evolve dynamically based on its trust score.
 * Higher trust → more capital access. Declining trust triggers recovery mode,
 * which caps the tier until a streak of good behaviour is demonstrated.
 *
 * Trust Tiers:
 *   probation (0–71)  → 40% capital,  3% max position
 *   limited   (72–81) → 70% capital,  6% max position
 *   standard  (82–89) → 90% capital,  8% max position
 *   elevated  (90–94) → 100% capital, 10% max position
 *   elite     (95+)   → 100% capital, 12% max position
 */

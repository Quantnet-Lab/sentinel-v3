 ...
 *
 * Human oversight controls for manual pause, resume, and emergency stop.
 * Supports EIP-1271 signature verification for authenticated operator commands.
 * All actions produce tamper-evident receipts for audit trail inclusion.
 */
 ...
 * If a signature is provided, verifies the operator's identity (EOA or
 * smart-contract wallet) before executing the action.
 */
export async function verifiedOperatorAction(
  ...
  if (verification) receipt.signatureVerification = verification;
  return receipt;
}

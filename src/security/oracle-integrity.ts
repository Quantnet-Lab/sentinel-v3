 ...
 *
 * Validates price feed health before any trade is allowed to proceed.
 * Catches: flash crashes, fat-finger anomalies, stale feeds, and
 * multi-bar cumulative manipulation attempts.
 *
 * Status levels:
 *   healthy → all checks pass, trading permitted
 *   watch   → soft warnings present, continue with caution
 *   blocked → hard blocker detected, halt trade this cycle
 */
...
export function evaluateOracleIntegrity(input: OracleIntegrityInput): OracleIntegrityResult {
  ...
  if (externalDeviationPct !== null) {
    if (externalDeviationPct > maxExternalDeviationPct)
      blockers.push(`external_deviation ${fmtPct(externalDeviationPct)} > ${fmtPct(maxExternalDeviationPct)}`);
    else if (externalDeviationPct > maxExternalDeviationPct * 0.7)
      reasons.push(`external_deviation_watch ${fmtPct(externalDeviationPct)}`);
  }

  if (singleBarMovePct > maxSingleBarMovePct)
    blockers.push(`single_bar_move ${fmtPct(singleBarMovePct)} > ${fmtPct(maxSingleBarMovePct)}`);
  else if (singleBarMovePct > maxSingleBarMovePct * 0.7)
    reasons.push(`single_bar_move_watch ${fmtPct(singleBarMovePct)}`);

  if (singleBarRangePct > maxSingleBarRangePct)
    blockers.push(`single_bar_range ${fmtPct(singleBarRangePct)} > ${fmtPct(maxSingleBarRangePct)}`);

  // Multi-bar cumulative move guard: catches slow-burn manipulation across 3 bars
  const maxMultiBarMovePct = 0.10;
  ...
  if (staleMinutes > staleAfterMinutes)
    blockers.push(`stale_price_feed ${staleMinutes.toFixed(1)}m > ${staleAfterMinutes}m`);
